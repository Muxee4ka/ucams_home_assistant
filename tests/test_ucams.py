...
CAMERA_FAKE_INFO = {
    ...
    "tariff": {"dvr_hours": 120, "dvr_size": None, "name": "Видеоархив 5"},
...
async def test_get_cameras_info(ucams_api):
    ...
    with aioresponses() as m:
        ...
        m.post(
            "https://cams.example.com/api/v0/auth/?ttl=20800", payload={"token": AUTH_FAKE_TOKEN}
        )
        # Мок ответа на запрос информации о камерах
        m.post(
            "https://cams.example.com/api/v0/cameras/my/", payload={"results": [CAMERA_FAKE_INFO]}
        )
        ...
        assert (
            ...
            == f"rtsp://flussonic-msk-1.cams.example.com/1234567890ABCDEF?token={CAMERA_FAKE_INFO['token_l']}&tracks=v1a1"
        )
        assert (
            cameras_info["1234567890ABCDEF"]["url_screen"]
            == f"https://ucams-screen-1.example.com/api/v0/screenshots/1234567890ABCDEF~600.jpg?token={CAMERA_FAKE_INFO['token_l']}"
        )
...
async def test_token_is_expired(ucams_api):
    ...
    with aioresponses() as m:
        ...
        assert (
            camera_url
            == f"rtsp://flussonic-msk-1.cams.example.com/{cam_fake_info['number']}?token={CAMERA_FAKE_INFO['token_l']}&tracks=v1a1"
        )
...
async def test_get_camera_archive(ucams_api):
    ...
    with aioresponses() as m:
        ...
        assert (
            archive_url
            == f"https://flussonic-msk-1.cams.example.com/{CAMERA_FAKE_INFO['number']}/archive-0-3600.mp4?token=token_d"
        )
        archive_url = await ucams_api.get_camera_archive(CAMERA_FAKE_INFO["number"], 0, 3700)
        assert (
            archive_url
            == f"https://flussonic-msk-1.cams.example.com/{CAMERA_FAKE_INFO['number']}/archive-0-3700.ts?token=token_d"
        )
